...
const DAY_HEADERS = ["月曜日", "火曜日", "水曜日", "木曜日", "金曜日", "土曜日", "日曜日"];

interface Props {
  /** 外部（音声入力など）から追加するイベントのキュー */
  pendingEvent?: CalendarEvent | null;
  onPendingConsumed?: () => void;
}

export default function Calendar({ pendingEvent, onPendingConsumed }: Props) {
  ...
  // 音声入力などの外部追加イベントを受け取って即時追加
  // pendingEvent が変化したら events に追加し、追加後はカレンダーをその月に移動
  ...
  return (
    <div className="flex flex-col h-screen bg-white font-sans pb-24">
      ...
      <header className="flex items-center px-4 py-2 border-b border-gray-200 bg-white shrink-0">
        <span className="text-teal-500 font-bold text-xl mr-6 select-none">✳ TimeTree</span>

        <Button variant="outline" size="sm" onClick={() => setCurrentDate(new Date())} className="mr-3">
          今日
        </Button>
        ...
        <span className="px-2 py-0.5 rounded bg-teal-500 text-white text-xs font-medium">月次</span>
                    ...
                    {visibleEvents.map((event) => {
                      ...
                      return (
                        <div
                          ...
                          title={event.title}
                          className={`text-xs py-0.5 px-1 cursor-pointer hover:opacity-80 transition-opacity truncate ${
                            event.isAllDay
